...
public class OperationEditorPaneController {
	...
	public Operation displayDialog(CompteCourant cpte, CategorieOperation mode) {
		...
		case DEBIT:
			...
			break;
		case CREDIT:
		    String creditInfo = "Cpt. : " + this.compteEdite.idNumCompte + "  "
		            + String.format(Locale.ENGLISH, "%12.02f", this.compteEdite.solde) + "  /  "
		            + String.format(Locale.ENGLISH, "%8d", this.compteEdite.debitAutorise);
		    this.lblMessage.setText(creditInfo);
	...
	private void doAjouter() {
		...
		case CREDIT:
		    ...
		    // - Le montant doit être un nombre valide et supérieur à 0
	...
	private void doAjouterEx() {
		switch (this.categorieOperation) {
			case DEBIT:
				...
				if (this.compteEdite.solde - montant < this.compteEdite.debitAutorise) {
					info = "Dépassement du découvert ! Débit exceptionnel autorisé quand meme - Cpt. : " + this.compteEdite.idNumCompte + "  "
							+ String.format(Locale.ENGLISH, "%12.02f", this.compteEdite.solde) + "  /  "
							+ String.format(Locale.ENGLISH, "%8d", this.compteEdite.debitAutorise);
					this.lblMessage.setText(info);
					this.txtMontant.getStyleClass().add("borderred");
