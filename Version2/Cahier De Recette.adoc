...
>|Titre 4+|Debiter/Crediter un compte
>|Objectif 4+| Vérifier qu'il est possible de debiter et ou crediter un compte

5+|

^|ID ^|Action ^|Comportement attendu ^|Pré-conditions ^|Résultat
^|All_G_CB.1 ^|Cliquer sur le bouton "Debiter/Crediter un compte". ^|La page des debits/credits s’affiche. ^|un compte a debiter crediter est selectionné ^|OK
^|All_G_CB.2 ^|crediter le compte de 100 euros. ^|le compte est credite de 100 euros ^|pas de precondition particulière ^|OK
...
|====
